...
class Instruct_Mapper:
    ...
    def reset(self,position,rotation):
        ...
        current_node = self.graph.add_node(our_Node([0, 0, 0]))
    ...
    def update(self,current_node,rgb,depth,rotation):
        self.current_position = self.graph.nodes[current_node].position
        self.current_rotation = self.rotation_func(rotation)
        self.current_depth = preprocess_depth(depth)
        print(np.max(self.current_depth), np.mean(self.current_depth), np.min(self.current_depth))
        ...
        masks: [num of classes * (H, W) of bool]
        confidences: [num of classes * float]
        '''
        ...
        self.object_entities, self.current_object_indices = self.associate_object_entities(self.object_entities,current_object_entities)
        ...
        #REVIEW whether comment this part
    ...
    def save_pointcloud_debug(self,path="./"):
        ...
        np.savetxt(path + "nodes.txt", self.nodes)
    ...
    def get_nodes(self, current_pcd, idx, step):
        print("fucufucufucu", self.current_position)
        ...
        pcd = current_pcd.select_by_index(
            (current_pcd.point.positions[:, 2] < self.floor_height).nonzero()[0])
        current_position = self.current_position

        current_navigable_position = pcd.point.positions.cpu().numpy()
        obstcale_pcd = current_pcd.select_by_index(
            (current_pcd.point.positions[:, 2] > self.floor_height + 0.1).nonzero()[0])
        obstcale_pcd = obstcale_pcd.select_by_index(
            (obstcale_pcd.point.positions[:, 2] < self.ceiling_height).nonzero()[0])
        obstcale_pcd_points = obstcale_pcd.point.positions.cpu().numpy()

        # change the z value of the points to the mean of the floor points
        current_navigable_position[:, 2] = np.ones_like(current_navigable_position[:, 2]) * np.mean(
        ...
        current_navigable_position = self.is_out_of_boundary_nav(current_navigable_position, obstcale_pcd_points, radius=0.1,
                                                            density_threshold=1)
        ...
        current_navigable_colors = np.ones_like(interpolate_points) * 100
        current_navigable_pcd = gpu_pointcloud_from_array(current_navigable_position, current_navigable_colors, self.pcd_device).voxel_down_sample(self.pcd_resolution)
        ...
        frontier_clusters, frontier_centers = (
            self.get_frontiers_offerd(obstcale_pcd, current_navigable_pcd, self.floor_height + 0.2, 0.1, closest_distance))
        ...
        if len(frontier_clusters) != 0:
            ...
            o3d.io.write_point_cloud(f'tmp/episode-{idx}/frontier_{step}.ply',save_pcd)
        ...
        intersections = calculate_intersections(current_navigable_position[:, :2], standing_position[:2], max_distance=max_distance)
        ...
        mask_tensor_in_circle = o3d.core.Tensor(np.where(1-mask)[0], o3d.core.Dtype.Int64, device=current_navigable_pcd.device)
        ...
        for i in range(max_label + 1):
            mask_idx_tensor = o3d.core.Tensor((labels == i).nonzero()[0], o3d.core.Dtype.Int64, device=current_navigable_pcd.device)
            ...
            if len(current_frontier_clusters) != 0 and len(cluster_point) > 20:
                ...
                merged_frontier_clusters, merged_frontier_centers = self.merge_frontier_with_visibility_1(cluster_point,
                                                                                                        current_frontier_clusters,
                                                                                                        current_frontier_centers)
                ...
                centers_new = [center for idx, center in enumerate(merged_frontier_centers) if idx not in center_idx_to_remove]
        ...
        import os
        os.makedirs(f'tmp/episode-{idx}', exist_ok=True)
        o3d.io.write_point_cloud(f'tmp/episode-{idx}/nav_{step}.ply',save_pcd)
        ...
        if len(self.nodes) == 0:
            ...
            if len(centers) == 0:
                ...
                for i in range(max_label + 1):
                    ...
                    if len(current_frontier_clusters) != 0 and len(cluster_point) > 20:
                        clusters.append(cluster)
                        # merge the frontier clusters
                        merged_frontier_clusters, merged_frontier_centers = self.merge_frontier_with_visibility_1(
                            cluster_point,
                            current_frontier_clusters,
                            current_frontier_centers)
            ...
            centers[:,2] = np.ones_like(centers[:,2]) * self.current_position[2]
            ...
            valid_centers = np.concatenate(([self.current_position], centers), axis=0)

            self.nodes = valid_centers
            self.nodes_state = np.concatenate((np.array([1]), np.zeros(len(centers))), axis=0)

        else:
            for center in centers:
                distance = np.linalg.norm(self.nodes[:,:2] - center[:2], axis=1)
        ...
        if len(valid_centers) > 0:
            ...
            for sphere in center_spheres:
                combined_sphere += sphere
            o3d.io.write_triangle_mesh(f'tmp/episode-{idx}/centers_{step}.ply',combined_sphere)
    ...
    def get_frontiers(self, current_navigable_pcd, real_boundary):
        ...
        frontier_centers = []
        for i in range(max_label + 1):
            mask_idx_tensor = o3d.core.Tensor((labels == i).nonzero()[0], o3d.core.Dtype.Int64, device=frontiers_pcd.device)
    ...
    def get_frontiers_offerd(self, obstacle_pcd, navigable_pcd, obstacle_height=-0.7,grid_resolution=0.1, closest_distance=1.6):
        frontiers = project_frontier(obstacle_pcd, navigable_pcd, obstacle_height, grid_resolution)

        frontiers[:, 2] = np.ones_like(frontiers[:, 2]) * np.mean(np.array(navigable_pcd.point.positions.cpu().numpy())[:, 2])
    ...
    def merge_frontier_with_visibility(self, cluster_points, frontier_clusters, frontier_centers):
        def is_visible(point1, point2):
            ...
            for point in interpolate_points:
                if self.is_out_of_boundary(cluster_points, point):
                    return False
    ...
    def merge_frontier_with_visibility_1(self, cluster_points, frontier_clusters, frontier_centers):
        def is_out_of_boundary_frontier_cluster(point_cloud, points, radius=0.1, density_threshold=1):
            # 这里假设有一个函数，检查点是否在点云的边界外
            # 例如可以使用点云的凸包或距离来判断
            radius = radius
            density_threshold = density_threshold
        ...
        def is_visible(point1, point2):
            interpolate_points = np.linspace(point1, point2, 100)
            if is_out_of_boundary_frontier_cluster(cluster_points, interpolate_points, radius=0.1, density_threshold=1):
                return False
    ...
    def get_candidate_nodes(self):
        print(f'Node state: {self.nodes_state}')
        return self.nodes[self.nodes_state == 0]
