...
class HM3D_Objnav_Agent:
    ...
    def reset(self):
        ...
        self.mapper.reset(self.env.sim.get_agent_state().sensor_states['rgb'].position,
                          self.env.sim.get_agent_state().sensor_states['rgb'].rotation)
    ...
    def rotate_panoramic(self, rotate_times=12):
        ...
        print("object indices")
        print(self.mapper.current_obj_indices)
    ...
    def concat_panoramic(self, images):
        ...
        for i in range(len(copy_images)):
            if i % 2 != 0:
                row = (i // 6)
                col = ((i % 6) // 2)
                copy_images[i] = cv2.putText(copy_images[i], "Direction %d" % i, (100, 100), cv2.FONT_HERSHEY_SIMPLEX,
                                             2, (255, 0, 0), 6, cv2.LINE_AA)
                background_image[10 * (row + 1) + row * height:10 * (row + 1) + row * height + height:,
                col * width + col * 10:col * width + col * 10 + width, :] = copy_images[i]
    ...
    def update_trajectory(self):
        ...
        topdown_image = cv2.cvtColor(colorize_draw_agent_and_fit_to_height(self.metrics['top_down_map'], 1024),
                                     cv2.COLOR_BGR2RGB)
        topdown_image = cv2.putText(topdown_image, 'Success:%.2f,SPL:%.2f,SoftSPL:%.2f,DTS:%.2f' % (
        self.metrics['success'], self.metrics['spl'], self.metrics['soft_spl'], self.metrics['distance_to_goal']),
                                    (0, 100), cv2.FONT_HERSHEY_SIMPLEX, 2, (0, 0, 0), 2, cv2.LINE_AA)
    ...
    def query_chainon(self):
        ...
        query_content = "<Navigation Instruction>:{}, <Previous Plan>:{}, <Semantic Clue>:{}".format(self.instruct_goal,
                                                                                                     "{" + self.trajectory_summary + "}",
                                                                                                     semantic_clue)
        ...
        if self.trajectory_summary == "":
            ...
            self.trajectory_summary = self.trajectory_summary + '-' + str(answer['Action']) + '-' + str(
                answer['Landmark'])
        return answer
    ...
    def query_gpt4v(self):
        ...
        text_content = "<Navigation Instruction>:{}\n <Sub Instruction>:{}".format(self.instruct_goal,
                                                                                   self.trajectory_summary.split("-")[
                                                                                       -2] + "-" +
                                                                                   self.trajectory_summary.split("-")[
                                                                                       -1])
    ...
    def make_plan(self, rotate=True, failed=False):
        ...
        self.affordance_pcd, self.colored_affordance_pcd = self.mapper.get_objnav_affordance_map(
            self.chainon_answer['Action'], self.chainon_answer['Landmark'], self.gpt4v_pcd, self.chainon_answer['Flag'],
            failure_mode=failed)
        ...
        if self.affordance_pcd.max() == 0:
            self.affordance_pcd, self.colored_affordance_pcd = self.mapper.get_objnav_affordance_map(
                self.chainon_answer['Action'], self.chainon_answer['Landmark'], self.gpt4v_pcd, False,
                failure_mode=failed)
            self.found_goal = False

        self.affordance_map, self.colored_affordance_map = project_costmap(self.mapper.navigable_pcd,
                                                                           self.affordance_pcd,
    ...
    def make_plan_mod(self, rotate=True, failed=False, initial=False, idx=None):
        ...
        print("\n --------------------------------------------------")
        for i in range(self.mapper.representation.node_cnt):
            print(self.mapper.representation.nodes[i].position)
            print(self.mapper.representation.nodes[i].objects)
        print("--------------------------------------------------\n")
        ...
        print("shit")
        print(self.mapper.current_position, self.mapper.current_obj_indices)
        self.mapper.representation.update_obj(self.mapper.current_position, self.mapper.current_obj_indices)


        self.waypoint_final = self.mapper.get_candidate_node()
        if self.waypoint_final is None:
            print(self.mapper.representation.get_nodes_states())
            print(self.mapper.representation.get_edges())
            # exit(0)
            return False
        print(f'Final waypoint: {self.waypoint_final}')
        self.mapper.change_state(self.waypoint_final)

        self.path = self.mapper.get_path(self.waypoint_final)
        ...
        # self.mapper.get_nodes(self.current_pcd, idx=idx, step=step)
        # self.mapper.get_nodes(self.mapper.process_obs_pcd, idx=idx, step=step)
        ...
        # self.waypoint = self.mapper.choose_waypoint(candidate_nodes)
        #
