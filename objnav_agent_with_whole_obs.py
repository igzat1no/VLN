...
class HM3D_Objnav_Agent:
    ...
    def reset(self):
        ...
        self.mapper.initialize(self.position, self.rotation)
        self.mapper.change_state(self.mapper.current_position)
    ...
    def update_trajectory(self):
        ...
        topdown_image = cv2.putText(
            topdown_image, 'Success:%.2f,SPL:%.2f,SoftSPL:%.2f,DTS:%.2f' %
            (self.metrics['success'], self.metrics['spl'], self.metrics['soft_spl'],
             self.metrics['distance_to_goal']), (0, 100), cv2.FONT_HERSHEY_SIMPLEX, 2, (0, 0, 0), 2,
            cv2.LINE_AA)
    ...
    def save_trajectory(self, dir="./tmp_objnav/"):
        ...
        for i, img, dep, seg, met in zip(np.arange(len(self.rgb_trajectory)), self.rgb_trajectory,
                                         self.depth_trajectory, self.segmentation_trajectory,
                                         self.topdown_trajectory):
    ...
    def query_chainon(self):
        ...
        query_content = "<Navigation Instruction>:{}, <Previous Plan>:{}, <Semantic Clue>:{}".format(
            self.instruct_goal, "{" + self.trajectory_summary + "}", semantic_clue)
        ...
        for i in range(10):
            try:
                ...
                if 'Action' in answer.keys() and 'Landmark' in answer.keys(
                ) and 'Flag' in answer.keys():
                    break
            except:
                continue
        ...
        if self.trajectory_summary == "":
            ...
            self.trajectory_summary = self.trajectory_summary + '-' + str(
                answer['Action']) + '-' + str(answer['Landmark'])
        return answer
    ...
    def query_gpt4v(self):
        ...
        text_content = "<Navigation Instruction>:{}\n <Sub Instruction>:{}".format(
            self.instruct_goal,
            self.trajectory_summary.split("-")[-2] + "-" + self.trajectory_summary.split("-")[-1])
    ...
    def make_plan_mod(self, rotate=True, failed=False, initial=False, idx=None):
        # print("\n --------------------------------------------------")
        # for i in range(self.mapper.node_cnt):
        #     print(self.mapper.nodes[i].position)
        #     print(self.mapper.nodes[i].objects)
        # print("--------------------------------------------------\n")

        print("before", self.mapper.current_position)
        print(self.position)
        # self.mapper.visit_node(self.mapper.current_position)
        self.rotate_panoramic()
        print("after", self.mapper.current_position)
        print(self.position)
        ...
        o3d.io.write_point_cloud(f'debug/episode-{idx}/obs_{step}.ply', save_pcd)

        print("\n \n --------------------------------------------------")
        print(f'Current step: {step}')
        print("before", self.mapper.node_cnt)
        self.mapper.get_nodes(self.current_pcd, idx=idx, step=step)
        print("after", self.mapper.node_cnt)
        print(self.mapper.get_nodes_states())
        print(self.mapper.get_nodes_positions())
        print("current position: ", self.mapper.current_position)
        self.mapper.update_obj(self.mapper.current_position,
                               self.mapper.current_obj_indices)

        self.waypoint_final = self.mapper.get_candidate_node()
        if self.waypoint_final is None:
            print(self.mapper.get_nodes_states())
            print(self.mapper.get_edges())
            exit(0)
            return False
